...
fn main() {
    ...
    for (i, tokens) in tokenized_form.iter().enumerate() {
        match &tokens[..] {
            [OP, REG] => {
                // let digit = u16::from_str_radix(&REG[..], 10).expect("couldn't parse digits");
                match &OP[..] {
                    "CLEAR" => match &REG[..] {
                        "A" => {
                    ...
                    "SUB" => match &REG[..] {
                        ...
                        i if i.starts_with("#") => {
                            let digit =
                                u16::from_str_radix(&i[1..], 10).expect("not a valid number");
                    ...
                    "PRNT" => {
                        hex_codes.push(0b0_001011_000000010);
                    }
    ...
    for (i, hex) in hex_codes.iter().enumerate() {
        write!(&mut new_file, "{:x}", hex).unwrap();

        if (i + 1) % 2 == 0 {
            write!(&mut new_file, " ").unwrap();
        }
    }
}
